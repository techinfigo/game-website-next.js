#!/usr/bin/env node
// One-time script: seed the Firestore "courses" collection from data/defaultCourses.ts.
// Deletes all existing "courses" documents first, then writes the 20 real courses.
//
// Usage:
...
import { DEFAULT_COURSES } from "../data/defaultCourses.ts";
...
function loadServiceAccount() {
  ...
  console.error(
    [
      ...
      "       FIREBASE_SERVICE_ACCOUNT_KEY='{\"type\":\"service_account\", ...}' node scripts/seed-courses.mjs",
      "",
...
async function seedCourses(db) {
  ...
  for (const course of DEFAULT_COURSES) {
    ...
    batch.set(coursesRef.doc(), docData);
    opsInBatch++;
    addedCount++;
...
async function main() {
  ...
  console.log('Deleting existing documents in "courses"...');
  const deletedCount = await deleteAllCourses(db);
  console.log(`Deleted ${deletedCount} existing document(s).`);

  console.log(`Seeding ${DEFAULT_COURSES.length} course(s) from data/defaultCourses.ts...`);
  const addedCount = await seedCourses(db);

  console.log(`Done. Deleted ${deletedCount} document(s), added ${addedCount} document(s) to "courses".`);
}
