#!/usr/bin/env node
// One-time script: seed the Firestore "offers" collection from data/offerData.ts.
// Deletes all existing "offers" documents first, then writes the single DEFAULT_OFFER.
//
// Usage:
...
import { DEFAULT_OFFER } from "../data/offerData.ts";
...
function loadServiceAccount() {
  ...
  console.error(
    [
      ...
      "       FIREBASE_SERVICE_ACCOUNT_KEY='{\"type\":\"service_account\", ...}' node scripts/seed-offers.mjs",
      "",
...
async function deleteAllOffers(db) {
...
async function seedOffer(db) {
  ...
  await offersRef.doc().set(docData);
  return 1;
}
...
async function main() {
  ...
  console.log('Deleting existing documents in "offers"...');
  const deletedCount = await deleteAllOffers(db);
  console.log(`Deleted ${deletedCount} existing document(s).`);

  console.log('Seeding 1 offer from data/offerData.ts...');
  const addedCount = await seedOffer(db);

  console.log(`Done. Deleted ${deletedCount} document(s), added ${addedCount} document(s) to "offers".`);
}
