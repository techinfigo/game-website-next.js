...
import { Users, Sparkles, ChevronRight, ChevronLeft, X } from 'lucide-react';
import { useFaculty } from '@/hooks/useFaculty';
import { DEFAULT_CHIEF_MENTOR, DEFAULT_FACULTY } from '@/data/facultyData';

// Kept for backward compatibility with any other modules importing these directly.
export const chiefMentor = DEFAULT_CHIEF_MENTOR;
export const facultyMembers = DEFAULT_FACULTY;
...
export default function EducatorsSection() {
  ...
  const { chiefMentor: liveChiefMentor, facultyMembers: liveFacultyMembers } = useFaculty();

  const totalSlides = Math.ceil(liveFacultyMembers.length / 2);
         ...
         <div className="max-w-[1280px] mx-auto px-6 md:px-10 lg:px-12 relative z-10 w-full" id="educators-container">
            ...
            <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 items-stretch">
               ...
               <motion.div 
                  ...
                  viewport={{ once: true }}
                  onClick={() => setSelectedFaculty(liveChiefMentor)}
               ...
               >
                  ...
                  <div className="h-[280px] lg:h-[340px] relative overflow-hidden">
                     <Image
                        src={liveChiefMentor.img}
                        alt={liveChiefMentor.name}
                        fill
                     ...
                     <div className="absolute bottom-4 left-6 right-6">
                        <div className="inline-flex items-center gap-2 px-2 py-1 bg-gameGold/20 border border-gameGold/30 rounded-lg mb-2">
                           ...
                           <span className="text-gameGold font-black text-[10px] uppercase tracking-widest">{liveChiefMentor.expLabel}</span>
                        </div>
                        <h3 className="text-2xl font-black text-white mb-1 leading-none tracking-tighter group-hover:text-gameGold transition-colors">
                           {liveChiefMentor.name}
                        </h3>
                        <p className="text-gameTeal font-black text-[10px] uppercase tracking-widest">{liveChiefMentor.role}</p>
                     </div>
                  </div>
                  ...
                  <div className="relative overflow-hidden flex-1">
                     ...
                     >
                        <AnimatePresence mode="wait">
                           {liveFacultyMembers.slice(facultySlide * 2, facultySlide * 2 + 2).map((fac, i) => (
                              <motion.div 
                                 key={fac.name} 
